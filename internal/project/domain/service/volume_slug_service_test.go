...
func TestVolumeSlugService_GenerateSlug(t *testing.T) {
	t.Run("successful slug generation", func(t *testing.T) {
		mockRepo := new(repository.MockVolumeRepository)
		...
		regex := regexp.MustCompile(`^v[0-9]{14}[a-z0-9]{8}$`)
...
func TestVolumeSlugService_EnsureUniqueSlug(t *testing.T) {
	t.Run("unique slug", func(t *testing.T) {
		mockRepo := new(repository.MockVolumeRepository)
		testSlug := "v2025011812000012345678"
