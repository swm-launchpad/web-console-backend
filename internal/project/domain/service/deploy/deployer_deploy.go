...
func (s *deployService) DeployProject(ctx context.Context, projectID uint) (*deployment.Deployment, error) {
	...
	// Step 2.5: Image tag validation
	// Note: This validation is no longer necessary as all containers now use "latest" as the default tag
	// for first-time builds instead of "pending". All containers will have a valid image tag.
	// Keeping this comment for historical context.
	// Previous behavior: Validated that containers were not using "pending" tag in standalone deploy.
